...
class Regenerator():
    ...
    def radius_search(self, x, y, k, r):
        ...
        select_num = k
        indices_total = torch.zeros(num_corr_key, select_num, dtype=torch.int64).to(device)
    ...
    def radius_search2(self, src_key, src_pts, tgt_key, tgt_pts, k):
        ...
        for i in range(num_corr_key):
            ...
            # elif indices.shape[0] < select_num:
            #     indices = torch.cat([indices, torch.ones(select_num-indices.shape[0], dtype=torch.int64).to(device)*indices[0]], dim=0)
    ...
    def local_matching(self, src_key_corr, tgt_key_corr, src_corr_knn_idx, tgt_corr_knn_idx, src_point, tgt_point, src_feature, tgt_feature):
        ...
        correspondences = torch.zeros((num_seeds, num_knn*2, 2), dtype=torch.int64).to(device)
        correspondences_true_mask = torch.zeros((num_seeds, num_knn*2), dtype=torch.bool).to(device)
    ...
    def match_pair_GMM(self, src_features, tgt_features):
        ...
        src_idx1 = torch.arange(num_knn)[None, :, None].expand(num_seeds,-1, num_k).to(device)
        bs_idx1 = torch.arange(num_seeds)[:, None, None].expand(-1, num_knn, num_k).to(device)

        mask_2_p2q = torch.zeros_like(distance, dtype=bool).to(device)
        mask_2_p2q[bs_idx1.reshape(-1), src_idx1.reshape(-1), tgt_idx1.reshape(-1)] = True
        mask_1_p2q = torch.zeros_like(distance, dtype=bool).to(device)
        ...
        tgt_idx2 = torch.arange(num_knn)[None, None, :].expand(num_seeds, num_k, -1).to(device)
        bs_idx2 = torch.arange(num_seeds)[:, None, None].expand(-1, num_k, num_knn).to(device)

        mask_2_q2p = torch.zeros_like(distance, dtype=bool).to(device)
        mask_2_q2p[bs_idx2.reshape(-1), src_idx2.reshape(-1), tgt_idx2.reshape(-1)] = True
        mask_1_q2p = torch.zeros_like(distance, dtype=bool).to(device)
    ...
    def regenerate(self, src_key_corr, tgt_key_corr, src_point, tgt_point, src_feature, tgt_feature, gt_trans, knn_num=100, sampling_num=100, knn_radius=0.8, use_sampling=False):
        ...
        total_num = src_key_corr.shape[1]
        rand_indices = torch.randint(0, total_num, (self.sampling_num * 2,), device=src_key_corr.device)
        sel_ind = torch.unique(rand_indices)[:self.sampling_num]
